...
export default class Page1804 extends MyFormComponent {
  state = {
    myTable1: {
      data: [],  //某一页数据
      total: 0,  //行总数
  ...
  handleDeleteRow = async (row, rowindex) => {  //ddddddddddddddelete   
    ...
    xdata._reloadrow = 0;
    let p = {};
    p.sqlprocedure = 'demo1804b'; // "demo502c";
    p.data = [];
    p.data.push(xdata);
  ...
  handleSaveClick = async () => {  //sssssssssssave
    ...
    data._treeflag = 0;
    let p = {};
    p.sqlprocedure = 'demo1804b';  //"demo504a";
    p.data = [];
    p.data.push(data);
    if (addoredit != 'add') {
    ...
    if (rs.rows.length > 0 && (rs.rows[0]._error == undefined || rs.rows[0]._error == '')) { //数据验证
      ...
      this.setState({ addoredit: 'update', myWin1: false, myTable1: { ...table, total, rowindex, pageno } }, () => {
        setTimeout(() => {
          this.loadTableData();
          myNotice('洒水已经保存，请刷新数据!', '', 200)
        })
      });
    } else {
      myNotice('洒水记录编码重复，记录保存失败！', 'error');  //
    }
  }
  ...
  handleSpotChange = (value, row) => {
    //console.log(value,row)
    this.myForm1.setFieldValue('driverno', row.driverno);
  }

  handleStreetChange = (value, row) => {
    //console.log(value,row)
    this.myForm1.setFieldValue('streetno', row.streetno);
  }
  ...
  render() {
    ...
    const columns = [
      ...
      { dataIndex: 'wrecordno', title: '洒水记录编号', width: '50px', align: 'center', fixed: 'left' },
      { dataIndex: 'license', title: '车牌', width: '120px', align: 'center', ellipsis: true },
      { dataIndex: 'cleanername', title: '环卫工人名称', width: '100px',align: 'center',fixed: 'left', ellipsis: true },
      { dataIndex: 'drivername', title: '司机姓名', width: '100px', align: 'center', ellipsis: true },
      { dataIndex: 'streetname', title: '街道名称', width: '100px', align: 'center', ellipsis: true },
      { dataIndex: 'way', title: '洒水方式', width: '100px', align: 'center', ellipsis: true },
      { dataIndex: 'amount', title: '洒水量', width: '100px', align: 'center', ellipsis: true },
      { dataIndex: 'waterdate', title: '植洒水日期被状况', width: '100px', align: 'center', ellipsis: true },
...

      {
        ...
        render: (text, record, index) => <>
          ...
          <Popconfirm okText='确定' cancelText='取消' overlayStyle={{ width: 350 }} title='系统提示' description={`是否确定删除"${record.productname}"的这个洒水记录？`}
    ...
    return (<>
      ...
      <Modal name='myWin1' title='洒水记录详细信息' open={this.state.myWin1} width={480} forceRender centered maskClosable={false}
        cancelText='关闭' onCancel={() => { this.setState({ myWin1: false }) }}
        styles={{ position: 'relative', padding: 0, body: { overflowY: 'scroll', height: '330px', width: '450px', padding: 0, margin: 0 } }}
        ...
        <div style={{ position: 'relative' }}>  {/* 添加一个层，否则文本框需要大于30px才能输入 */}
          <Form name="myForm1" ref={ref => this.myForm1 = ref} autoComplete="off" onFinish={this.onFinish} initialValues={this.state.row}
            ...
            <AntdInputBox type='text' id='wrecordno' ref={ref => this.wrecordno = ref} label='记录编码' labelwidth='100' left='2' width='140' top={8 + rowheight * 0} />
            <AntdComboBox id='driverno' label='司机名称' labelwidth='100' left='2' width='300' top={2 + rowheight * 1} sqlprocedure='demo1804d' onChange={this.handleSpotChange} textfield='drivername' valuefield='driverno' />
            <AntdComboBox id='cleanerno' label='环卫工人名称' labelwidth='100' left='2' width='300' top={2 + rowheight * 2} sqlprocedure='demo1804c' onChange={this.handleSupplierChange} textfield='cleanername' valuefield='cleanerno' />
            <AntdComboBox id='sprinklerno' label='车牌号' labelwidth='100' left='2' width='300' top={2 + rowheight * 3} sqlprocedure='demo1804f' onChange={this.handleLicenseChange} textfield='license' valuefield='sprinklerno' />
            <AntdComboBox id='streetno' label='街道名称' labelwidth='100' left='2' width='300' top={2 + rowheight * 4} sqlprocedure='demo1804e' onChange={this.handleStreetChange} textfield='streetname' valuefield='streetno' />
            <AntdInputBox type='text' id='way' ref={ref => this.way = ref} label='洒水方式' labelwidth='100' left='2' width='300' top={8 + rowheight * 5} />
            <AntdInputBox type='text' id='amount' ref={ref => this.amount = ref} label='洒水量' labelwidth='100' left='2' width='300' top={8 + rowheight * 6} />
            <AntdInputBox type='text' id='waterdate' ref={ref => this.waterdate = ref} label='洒水日期' labelwidth='100' left='2' width='300' top={8 + rowheight * 7} />
          </Form>
        </div>
      </Modal>
