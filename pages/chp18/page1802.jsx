...
export default class Page1802 extends MyFormComponent {
  state = {
    myTable1: {
      data: [],  //某一页数据
      total: 0,  //行总数
  ...
  handleDeleteRow = async (row, rowindex) => {  //ddddddddddddddelete   
    ...
    xdata._reloadrow = 0;
    let p = {};
    p.sqlprocedure = 'demo1802b'; // "demo502c";
    p.data = [];
    p.data.push(xdata);
  ...
  handleSaveClick = async () => {  //sssssssssssave
    ...
    data._treeflag = 0;
    let p = {};
    p.sqlprocedure = 'demo1802b';  //"demo504a";
    p.data = [];
    p.data.push(data);
    if (addoredit != 'add') {
    ...
    if (rs.rows.length > 0 && (rs.rows[0]._error == undefined || rs.rows[0]._error == '')) { //数据验证
      ...
      this.setState({ addoredit: 'update', myWin1: false, myTable1: { ...table, total, rowindex, pageno } }, () => {
        setTimeout(() => {
          this.loadTableData();
          myNotice('绿化记录记录已经保存，请刷新数据!', '', 200)
        })
      });
    } else {
      myNotice('绿化记录编码重复，记录保存失败！', 'error');  //
    }
  }
  ...
  render() {
    ...
    const columns = [
      ...
      { dataIndex: 'detectno', title: '检测记录编号', width: '100px', align: 'center', fixed: 'left' },
      { dataIndex: 'streetname', title: '街道名称', width: '80px', align: 'center',fixed: 'left', ellipsis: true },

      { dataIndex: 'monitorno', title: '监测点编号', width: '80px', align: 'center', ellipsis: true },
      { dataIndex: 'visibility', title: '能见度（米）', width: '100px',align: 'center', ellipsis: true },
      { dataIndex: 'detectitems', title: '其余指标测量数据', width: '450px', align: 'center', ellipsis: true },
      { dataIndex: 'detecttime', title: '监测时间', width: '150px', align: 'center', ellipsis: true },
      { dataIndex: 'temperature', title: '温度', width: '80px', align: 'center', ellipsis: true },
...

      {
        ...
        render: (text, record, index) => <>
          ...
          <Popconfirm okText='确定' cancelText='取消' overlayStyle={{ width: 350 }} title='系统提示' description={`是否确定删除"${record.productname}"的这个绿化记录？`}
    ...
    return (<>
      ...
      <Modal name='myWin1' title='绿化记录详细信息' open={this.state.myWin1} width={480} forceRender centered maskClosable={false}
        cancelText='关闭' onCancel={() => { this.setState({ myWin1: false }) }}
        styles={{ position: 'relative', padding: 0, body: { overflowY: 'scroll', height: '300px', width: '450px', padding: 0, margin: 0 } }}
        ...
        <div style={{ position: 'relative' }}>  {/* 添加一个层，否则文本框需要大于30px才能输入 */}
          <Form name="myForm1" ref={ref => this.myForm1 = ref} autoComplete="off" onFinish={this.onFinish} initialValues={this.state.row}
            ...
            <AntdInputBox type='text' id='detectno' ref={ref => this.detectno = ref} label='记录编码' labelwidth='100' left='2' width='300' top={8 + rowheight * 0} />
            <AntdInputBox type='text' id='monitorno' ref={ref => this.monitorno = ref} label='监测点编码' labelwidth='100' left='2' width='300' top={8 + rowheight * 1} />
            <AntdInputBox type='text' id='visibility' ref={ref => this.visibility = ref} label='能见度' labelwidth='100' left='2' width='300' top={8 + rowheight * 2} />
            <AntdInputBox type='text' id='humidity' ref={ref => this.humidity = ref} label='湿度' labelwidth='100' left='2' width='300' top={8 + rowheight * 3} />

            <AntdInputBox type='text' id='detectitems' ref={ref => this.detectitems = ref} label='数据明细' labelwidth='100' left='2' width='300' top={8 + rowheight * 4} />

            <AntdInputBox type='text' id='detecttime' ref={ref => this.detecttime = ref} label='检测时间' labelwidth='100' left='2' width='300' top={8 + rowheight * 5} />
            <AntdInputBox type='text' id='temperature' ref={ref => this.temperature = ref} label='温度' labelwidth='100' left='2' width='300' top={8 + rowheight * 6} />
            <AntdInputBox type='text' id='streetno' ref={ref => this.streetno = ref} label='街道编号' labelwidth='100' left='2' width='300' top={8 + rowheight * 7} />
          </Form>
        </div>
      </Modal>
