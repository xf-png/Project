...
export default class Page1404 extends MyFormComponent {
  state = {
    myTable1: {
      data: [],  //某一页数据
      total: 0,  //行总数
  ...
  handleDeleteClick = async (e) => {
    ...
    this.myDeleteModal.setState({ visible: true, description: '是否确定删除【' + row[keyfield] + '】这个环卫站？' });
    return;
  }
  ...
  handleDeleteRow = async (row, rowindex) => {  //ddddddddddddddelete   
    ...
    xdata._reloadrow = 0;
    let p = {};
    p.sqlprocedure = 'demo1404b'; // "demo502c";
    p.data = [];
    p.data.push(xdata);
  ...
  handleSaveClick = async () => {  //sssssssssssave
    ...
    data._treeflag = 0;
    let p = {};
    p.sqlprocedure = 'demo1404b';  //"demo504a";
    p.data = [];
    p.data.push(data);
    if (addoredit != 'add') {
    ...
    if (rs.rows.length > 0 && (rs.rows[0]._error == undefined || rs.rows[0]._error == '')) { //数据验证
      ...
      this.setState({ addoredit: 'update', myWin1: false, myTable1: { ...table, total, rowindex, pageno } }, () => {
        setTimeout(() => {
          this.loadTableData();
          myNotice('环卫站记录已经保存，请刷新数据!', '', 200)
        })
      });
    } else {
      myNotice('环卫站编码重复，记录保存失败！', 'error');  //
    }
  }
  ...
  render() {
    ...
    const columns = [
      ...
      { dataIndex: 'streetno', title: '街道编号', width: '100px', align: 'center', ellipsis: true },// elipsis: true,溢出隐藏
      { dataIndex: 'stationname', title: '环卫站名称', width: '100px', align: 'center' },
      { dataIndex: 'telephone', title: '联系电话', width: '150px',align: 'center' },
      { dataIndex: 'employeenumber', title: '员工数量', width: '80px', align: 'center' },
      { dataIndex: 'sprnumber', title: '洒水车数量', width: '80px', align: 'center' },
      { dataIndex: 'address', title: '具体地址', width: '80px', align: 'center' },
...

      {
        ...
        render: (text, record, index) => <>
          ...
          <Popconfirm okText='确定' cancelText='取消' overlayStyle={{ width: 350 }} title='系统提示' description={`是否确定删除"${record.productname}"的这个环卫站？`}
    ...
    return (<>
      ...
      <Modal name='myWin1' title='环卫站详细信息' open={this.state.myWin1} width={480} forceRender centered maskClosable={false}
        ...
        <div style={{ position: 'relative' }}>  {/* 添加一个层，否则文本框需要大于30px才能输入 */}
          <Form name="myForm1" ref={ref => this.myForm1 = ref} autoComplete="off" onFinish={this.onFinish} initialValues={this.state.row}
            ...
            <AntdInputBox type='text' id='stationno' ref={ref => this.sprinklerno = ref} label='环卫站编号' labelwidth='82' left='2' width='140' top={8 + rowheight * 0} />
            <AntdInputBox type='text' id='streetno' ref={ref => this.sprinklername = ref} label='街道编号' labelwidth='82' left='2' width='300' top={8 + rowheight * 1} />
            <AntdInputBox type='text' id='stationname' ref={ref => this.stationno = ref} label='环卫站名称' labelwidth='82' left='2' width='300' top={8 + rowheight * 2} />
            <AntdInputBox type='text' id='telephone' ref={ref => this.brand = ref} label='联系电话' labelwidth='82' left='2' width='300' top={8 + rowheight * 3} />
            <AntdInputBox type='text' id='address' ref={ref => this.website = ref} label='具体地址' labelwidth='82' left='2' width='140' top={8 + rowheight * 4} />
            <AntdInputBox type='text' id='employeenumber' ref={ref => this.pumptype = ref} label='员工数量' labelwidth='82' left='2' width='85' top={8 + rowheight * 5} min={0.01} precision={2} controls={false} />
            <AntdInputBox type='text' id='sprnumber' ref={ref => this.waterstorage = ref} label='洒水车数量' labelwidth='82' left='2' width='140' top={8 + rowheight * 6 }/> 
            <AntdInputBox type='text' id='address' ref={ref => this.waterstorage = ref} label='环卫站地址' labelwidth='82' left='2' width='140' top={8 + rowheight * 7 }/> 
