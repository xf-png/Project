...
export default class Page1401 extends MyFormComponent {
  state = {
    myTable1: {
      data: [],  //某一页数据
      total: 0,  //行总数
  ...
  handleDeleteClick = async (e) => {
    ...
    this.myDeleteModal.setState({ visible: true, description: '是否确定删除【' + row[keyfield] + '】这个环卫工人？' });
    return;
  } 
  ...
  handleDeleteRow = async (row, rowindex) => {  //ddddddddddddddelete   
    ...
    xdata._reloadrow = 0;
    let p = {};
    p.sqlprocedure = 'demo1401b'; // "demo502c";
    p.data = [];
    p.data.push(xdata);
  ...
  handleSaveClick = async () => {  //sssssssssssave
    ...
    data._treeflag = 0;
    let p = {};
    p.sqlprocedure = 'demo1401b';  //"demo504a";
    p.data = [];
    p.data.push(data);
    if (addoredit != 'add') {
    ...
    if (rs.rows.length > 0 && (rs.rows[0]._error == undefined || rs.rows[0]._error == '')) { //数据验证
      ...
      this.setState({ addoredit: 'update', myWin1: false, myTable1: { ...table, total, rowindex, pageno } }, () => {
        setTimeout(() => {
          this.loadTableData();
          myNotice('工人记录已经保存，请刷新数据!', '', 200)
        })
      });
    } else {
      myNotice('工人编码重复，记录保存失败！', 'error');  //
    }
  }
  ...
  handleSupplierChange = (value, row) => {
    ...
    this.myForm1.setFieldValue('stationno', row.stationno);
  }
  ...
  render() {
    ...
    const columns = [
      ...
      { dataIndex: 'cleanerno', title: '员工编号', width: '120px', align: 'center', fixed: 'left' },
      { dataIndex: 'cleanername', title: '员工姓名', width: '120px',align: 'center',fixed: 'left', ellipsis: true },
      { dataIndex: 'idcard', title: '身份证号', width: '200px',align: 'center', ellipsis: true },
      { dataIndex: 'stationno', title: '环卫站编号', width: '100px', align: 'center', ellipsis: true },
      { dataIndex: 'stationname', title: '环卫站名称', width: '130px', align: 'center', ellipsis: true },// elipsis: true,溢出隐藏
      { dataIndex: 'gender', title: '性别', width: '50px',align: 'center' },
      { dataIndex: 'mobile', title: '手机号', width: '120px', align: 'center' },
      { dataIndex: 'birthdate', title: '出生日期', width: '90px', align: 'center' },
      { dataIndex: 'hiredate', title: '上岗日期', width: '90px' ,align: 'center' },
      { dataIndex: 'pays', title: '薪资', width: '50px', ellipsis: true ,align: 'center'},
      { dataIndex: 'address', title: '家庭地址', width: '450px', align: 'center' },
      { dataIndex: 'title', title: '职位', width: '100px', align: 'center' },
      {
        dataIndex: 'email', title: '邮箱', width: '150px', ellipsis: true,
        render: (text, record, index) => { return <b>{record.email}</b> }
      },
      {
        ...
        render: (text, record, index) => <>
          ...
          <Popconfirm okText='确定' cancelText='取消' overlayStyle={{ width: 350 }} title='系统提示' description={`是否确定删除"${record.productname}"的这个环卫工人？`}
    ...
    return (<>
      ...
      <Modal name='myWin1' title='环卫工人详细信息' open={this.state.myWin1} width={480} forceRender centered maskClosable={false}
        ...
        <div style={{ position: 'relative' }}>  {/* 添加一个层，否则文本框需要大于30px才能输入 */}
          <Form name="myForm1" ref={ref => this.myForm1 = ref} autoComplete="off" onFinish={this.onFinish} initialValues={this.state.row}
            ...
            <AntdInputBox type='text' id='cleanerno' ref={ref => this.cleanerno = ref} label='环卫工人编码' labelwidth='100' left='2' width='300' top={8 + rowheight * 0} />
            <AntdInputBox type='text' id='cleanername' ref={ref => this.cleanername = ref} label='环卫工人名称' labelwidth='100' left='2' width='300' top={8 + rowheight * 1} />
            <AntdInputBox type='text' id='idcard' ref={ref => this.idcard = ref} label='身份证号' labelwidth='100' left='2' width='300' top={8 + rowheight * 2} />

            <AntdInputBox type='text' id='stationno' ref={ref => this.stationno = ref} label='环卫站编号' labelwidth='100' left='2' width='300' top={8 + rowheight * 3} />
            <AntdComboBox id='stationno' ref={ref => this.stationno = ref} label='环卫站名称' labelwidth='100' left='2' width='300' top={8 + rowheight * 4} sqlprocedure='demo1401c' onChange={this.handleSupplierChange} textfield='stationname' valuefield='stationno' />

            <AntdInputBox type='text' id='gender' ref={ref => this.gender = ref} label='性别' labelwidth='100' left='2' width='300' top={8 + rowheight * 5} />
            <AntdInputBox type='text' id='mobile' ref={ref => this.mobile = ref} label='手机电话' labelwidth='100' left='2' width='140' top={8 + rowheight * 6} />
            <AntdInputBox type='text' id='birthdate' ref={ref => this.birthdate = ref} label='出生日期' labelwidth='100' left='216' width='85' top={8 + rowheight * 6} min={0.01} precision={2} controls={false} />
            <AntdInputBox type='text' id='hiredate' ref={ref => this.hiredate = ref} label='上岗日期' labelwidth='100' left='2' width='300' top={8 + rowheight * 7} />
            <AntdInputBox type='text' id='pays' ref={ref => this.pays = ref} label='薪资' labelwidth='100' left='2' width='300' readOnly top={8 + rowheight * 8} />
            <AntdInputBox type='text' id='address' ref={ref => this.address = ref} label='家庭地址' labelwidth='100' left='2' width='300' readOnly top={8 + rowheight * 9} />
            <AntdFileUpload id='photopath' ref={ref => this.photopath = ref} label='图片预览' labelwidth='100' left='2' width='300' top={8 + rowheight * 10}
              datatype='json' targetpath='mybase/resources/' listType='picture-card' type='image'
              fieldNames={{ filename: 'filename', filetitle: 'title' }} filetag='`${productid}_`'
              maxCount='5' />
