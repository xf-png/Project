...
export default class Page1403 extends MyFormComponent {
  state = {
    myTable1: {
      data: [],  //某一页数据
      total: 0,  //行总数
  ...
  handleDeleteClick = async (e) => {
    ...
    this.myDeleteModal.setState({ visible: true, description: '是否确定删除【' + row[keyfield] + '】这个洒水车？' });
    return;
  }
  ...
  handleDeleteRow = async (row, rowindex) => {  //ddddddddddddddelete   
    ...
    xdata._reloadrow = 0;
    let p = {};
    p.sqlprocedure = 'demo1403b'; // "demo502c";
    p.data = [];
    p.data.push(xdata);
  ...
  handleSaveClick = async () => {  //sssssssssssave
    ...
    data._treeflag = 0;
    let p = {};
    p.sqlprocedure = 'demo1403b';  //"demo504a";
    p.data = [];
    p.data.push(data);
    if (addoredit != 'add') {
    ...
    if (rs.rows.length > 0 && (rs.rows[0]._error == undefined || rs.rows[0]._error == '')) { //数据验证
      ...
      this.setState({ addoredit: 'update', myWin1: false, myTable1: { ...table, total, rowindex, pageno } }, () => {
        setTimeout(() => {
          this.loadTableData();
          myNotice('洒水车记录已经保存，请刷新数据!', '', 200)
        })
      });
    } else {
      myNotice('洒水车编码重复，记录保存失败！', 'error');  //
    }
  }
  ...
  handleSupplierChange = (value, row) => {
    ...
    this.myForm1.setFieldValue('sprinklerno', row.sprinklerno);
  }
  ...
  render() {
    ...
    const columns = [
      ...
      { dataIndex: 'license', title: '车牌号', width: '130px', fixed: 'left', ellipsis: true ,align: 'center'},
      { dataIndex: 'sprinklertype', title: '洒水车类型', width: '100px',align: 'center',ellipsis: true },
      { dataIndex: 'stationno', title: '环卫站编号', width: '100px', align: 'center', ellipsis: true },// elipsis: true,溢出隐藏
      { dataIndex: 'stationname', title: '环卫站名称', width: '150px', align: 'center' },
      { dataIndex: 'brand', title: '品牌', width: '150px',align: 'center' },
      { dataIndex: 'website', title: '品牌网站', width: '300px', align: 'center' },
      { dataIndex: 'pumptype', title: '水泵型号', width: '90px', align: 'center' },
      { dataIndex: 'waterstorage', title: '储水量（吨）', width: '100px' ,align: 'center' },

      { dataIndex: 'producedate', title: '生产日期', width: '130px', ellipsis: true ,align: 'center'},
      { dataIndex: 'serviceyear', title: '使用年限', width: '80px', align: 'center' },

      {
        ...
        render: (text, record, index) => <>
          ...
          <Popconfirm okText='确定' cancelText='取消' overlayStyle={{ width: 350 }} title='系统提示' description={`是否确定删除"${record.productname}"的这个洒水车？`}
    ...
    return (<>
      ...
      <Modal name='myWin1' title='洒水车详细信息' open={this.state.myWin1} width={480} forceRender centered maskClosable={false}
        cancelText='关闭' onCancel={() => { this.setState({ myWin1: false }) }}
        styles={{ position: 'relative', padding: 0, body: { overflowY: 'scroll', height: '430px', width: '450px', padding: 0, margin: 0 } }}
        ...
        <div style={{ position: 'relative' }}>  {/* 添加一个层，否则文本框需要大于30px才能输入 */}
          <Form name="myForm1" ref={ref => this.myForm1 = ref} autoComplete="off" onFinish={this.onFinish} initialValues={this.state.row}
            ...
            <AntdInputBox type='text' id='sprinklerno' ref={ref => this.sprinklerno = ref} label='洒水车编号' labelwidth='82' left='2' width='300' top={8 + rowheight * 0} />
            <AntdInputBox type='text' id='license' ref={ref => this.sprinklername = ref} label='车牌号' labelwidth='82' left='2' width='300' top={8 + rowheight * 1} />
            <AntdInputBox type='text' id='stationno' ref={ref => this.stationno = ref} label='环卫站编号' labelwidth='82' left='2' width='300' top={8 + rowheight * 2} />
            <AntdComboBox id='stationno' ref={ref => this.stationno = ref} label='环卫站名称' labelwidth='82' left='2' width='300' top={8 + rowheight * 3} sqlprocedure='demo1401c' onChange={this.handleSupplierChange} textfield='stationname' valuefield='stationno' />
            <AntdInputBox type='text' id='brand' ref={ref => this.brand = ref} label='品牌' labelwidth='82' left='2' width='300' top={8 + rowheight * 4} />
            <AntdInputBox type='text' id='website' ref={ref => this.website = ref} label='计量单位' labelwidth='82' left='2' width='300' top={8 + rowheight * 5} />
            <AntdInputBox type='text' id='pumptype' ref={ref => this.pumptype = ref} label='水泵种类' labelwidth='82' left='2' width='300' top={8 + rowheight * 6} min={0.01} precision={2} controls={false} />
            <AntdInputBox type='text' id='sprinklertype' ref={ref => this.waterstorage = ref} label='洒水车类型' labelwidth='82' left='2' width='300' top={8 + rowheight * 7} />
            <AntdInputBox type='text' id='producedate' ref={ref => this.producedate = ref} label='生产日期' labelwidth='82' left='2' width='300' readOnly top={8 + rowheight * 8} />
            <AntdInputBox type='text' id='serviceyear' ref={ref => this.serviceyear = ref} label='使用年限' labelwidth='82' left='2' width='300' readOnly top={8 + rowheight * 9} />
            <AntdHiddenField id='sprinklernox' />
