...
export default class Page1701 extends MyFormComponent {
  state = {
    myTable1: {
      data: [],  //某一页数据
      total: 0,  //行总数
  ...
  handleDeleteClick = async (e) => {
    ...
    this.myDeleteModal.setState({ visible: true, description: '是否确定删除【' + row[keyfield] + '】这个绿化点？' });
    return;
  }
  ...
  handleDeleteRow = async (row, rowindex) => {  //ddddddddddddddelete   
    ...
    xdata._reloadrow = 0;
    let p = {};
    p.sqlprocedure = 'demo1701b'; // "demo502c";
    p.data = [];
    p.data.push(xdata);
  ...
  handleSaveClick = async () => {  //sssssssssssave
    ...
    data._treeflag = 0;
    let p = {};
    p.sqlprocedure = 'demo1701b';  //"demo504a";
    p.data = [];
    p.data.push(data);
    if (addoredit != 'add') {
    ...
    if (rs.rows.length > 0 && (rs.rows[0]._error == undefined || rs.rows[0]._error == '')) { //数据验证
      ...
      this.setState({ addoredit: 'update', myWin1: false, myTable1: { ...table, total, rowindex, pageno } }, () => {
        setTimeout(() => {
          this.loadTableData();
          myNotice('绿化点记录已经保存，请刷新数据!', '', 200)
        })
      });
    } else {
      myNotice('绿化点编码重复，记录保存失败！', 'error');  //
    }
  }
  ...
  render() {
    ...
    const columns = [
      ...
      { dataIndex: 'spotname', title: '绿化点名称', width: '120px',align: 'center',fixed: 'left', ellipsis: true },
      { dataIndex: 'greentype', title: '绿化点类型', width: '100px', align: 'center', ellipsis: true },
      { dataIndex: 'square', title: '占地面积', width: '100px', align: 'center', ellipsis: true },
      { dataIndex: 'address', title: '详细地址', width: '100px', align: 'center', ellipsis: true },
...

      {
        ...
        render: (text, record, index) => <>
          ...
          <Popconfirm okText='确定' cancelText='取消' overlayStyle={{ width: 350 }} title='系统提示' description={`是否确定删除"${record.productname}"的这个绿化点？`}
    ...
    return (<>
      ...
      <Modal name='myWin1' title='绿化点详细信息' open={this.state.myWin1} width={480} forceRender centered maskClosable={false}
        ...
        <div style={{ position: 'relative' }}>  {/* 添加一个层，否则文本框需要大于30px才能输入 */}
          <Form name="myForm1" ref={ref => this.myForm1 = ref} autoComplete="off" onFinish={this.onFinish} initialValues={this.state.row}
            ...
            <AntdInputBox type='text' id='spotno' ref={ref => this.spotno = ref} label='绿化点编码' labelwidth='100' left='2' width='140' top={8 + rowheight * 0} />
            <AntdInputBox type='text' id='spotname' ref={ref => this.spotname = ref} label='绿化点名称' labelwidth='100' left='2' width='300' top={8 + rowheight * 1} />
            <AntdInputBox type='text' id='greentype' ref={ref => this.greentype = ref} label='绿化点类型' labelwidth='100' left='2' width='300' top={8 + rowheight * 2} />
            <AntdInputBox type='text' id='square' ref={ref => this.square = ref} label='占地面积' labelwidth='100' left='2' width='300' top={8 + rowheight * 3} />
            <AntdInputBox type='text' id='address' ref={ref => this.address = ref} label='详细地址' labelwidth='100' left='2' width='300' top={8 + rowheight * 4} />
