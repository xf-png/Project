...
export default class Page1704 extends Component {
  constructor(props) {
    super(props);
    //在这里初始化 state
    this.state = {      
  ...
  showOrders = async (item) => {  //必须加异步
    ...
    p.sqlprocedure='demo1704c';
    let rs = await reqdoSQL(p); //调用函数，执行存储过程。必须加await
    let rows=rs.rows.map((item, index) => {
      return(<li key={"item_"+index}>
        <span style={{display:'inline-block', width:70}}>{item.mowno}</span>
        <span style={{display:'inline-block', width:170}}>{item.spotname}</span>
        <span style={{display:'inline-block', width:100}}>{item.mowdate}</span>

        <span style={{display:'inline-block', width:80}}>{item.mowtime}</span>
        <span style={{display:'inline-block', width:70}}>{item.mowlong}</span>
        <span style={{display:'inline-block', width:70}}>{item.mowtools}</span>
        &nbsp;&nbsp;
        <span style={{display:'inline-block', width:70}}>{item.mowcondition}</span>
    ...
    <div>
      <div style={{height:30, marginLeft:10, marginTop:10}}>查询员工工作记录<hr/></div>      
        ...
        <div style={{padding:10, marginTop:10, border:'1px solid #95B8E7', height:260, overflow:'auto'}}>
          <div><b>{this.state.selectedProduct.cleanername}</b>修剪信息（共***条）</div>
          <hr/>

          <span>序号</span>&nbsp;<span>修剪记录编号</span>&nbsp;&nbsp;<span>绿化点名称</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span>修剪日期</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span>修剪时间</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span>用时</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span>使用工具</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span>植被状态</span>
