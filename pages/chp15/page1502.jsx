...
export default class Page1402 extends MyFormComponent {
  ...
  handleDeleteClick = async (e) => {
    ...
    this.myDeleteModal.setState({ visible: true, description: '是否确定删除【' + row[keyfield] + '】这个记录？' });
    return;
  }
  ...
  handleDeleteRow = async (row, rowindex) => {  //ddddddddddddddelete   
    ...
    xdata._reloadrow = 0;
    let p = {};
    p.sqlprocedure = 'demo1502b'; // "demo502c";
    p.data = [];
    p.data.push(xdata);
  ...
  handleSaveClick = async () => {  //sssssssssssave
    ...
    data._treeflag = 0;
    let p = {};
    p.sqlprocedure = 'demo1502b';  //"demo504a";
    p.data = [];
    p.data.push(data);
    if (addoredit != 'add') {
    ...
    if (rs.rows.length > 0 && (rs.rows[0]._error == undefined || rs.rows[0]._error == '')) { //数据验证
      ...
      this.setState({ addoredit: 'update', myWin1: false, myTable1: { ...table, total, rowindex, pageno } }, () => {
        setTimeout(() => {
          this.loadTableData();
          myNotice('清扫记录已经保存，请刷新数据!', '', 200)
        })
      });
    } else {
      myNotice('清扫编码重复，记录保存失败！', 'error');  //
    }
  }
  ...
  render() {
    ...
    const columns = [
      ...
      { dataIndex: 'cleanername', title: '环卫工人名称', width: '120px',align: 'center',fixed: 'left', ellipsis: true },
      { dataIndex: 'streetname', title: '街道名称', width: '100px', align: 'center', ellipsis: true },
      { dataIndex: 'sweepdate', title: '清扫日期', width: '130px', align: 'center', ellipsis: true },// elipsis: true,溢出隐藏
      { dataIndex: 'sweeptime', title: '清扫时间', width: '50px',align: 'center' },
      { dataIndex: 'status', title: '完成情况', width: '120px', align: 'center' },
      
      {
        ...
        render: (text, record, index) => <>
          ...
          <Popconfirm okText='确定' cancelText='取消' overlayStyle={{ width: 350 }} title='系统提示' description={`是否确定删除"${record.productname}"的这个记录？`}
    ...
    return (<>
      ...
      <Modal name='myWin1' title='记录详细信息' open={this.state.myWin1} width={480} forceRender centered maskClosable={false}
        ...
        <div style={{ position: 'relative' }}>  {/* 添加一个层，否则文本框需要大于30px才能输入 */}
          <Form name="myForm1" ref={ref => this.myForm1 = ref} autoComplete="off" onFinish={this.onFinish} initialValues={this.state.row}
            ...
            <AntdInputBox type='text' id='srecordno' ref={ref => this.srecordno = ref} label='记录编码' labelwidth='100' left='2' width='140' top={8 + rowheight * 0} />
            <AntdComboBox id='cleanerno' ref={ref => this.stationno = ref} label='环卫工人名称' labelwidth='100' left='2' width='300' top={8 + rowheight * 1} sqlprocedure='demo1502c' onChange={this.handleSupplierChange} textfield='cleanername' valuefield='cleanerno' />
            <AntdComboBox id='streetno' ref={ref => this.stationno = ref} label='街道名称' labelwidth='100' left='2' width='300' top={8 + rowheight * 2} sqlprocedure='demo1502d' onChange={this.handleSupplierChange} textfield='streetname' valuefield='streetno' />
            <AntdInputBox type='date' id='sweepdate' ref={ref => this.sweepdate = ref} label='清扫日期' labelwidth='100' left='2' width='140' top={8 + rowheight * 3} />
            <AntdInputBox type='text' id='sweeptime' ref={ref => this.sweeptime = ref} label='清扫时间' labelwidth='100' left='2' width='140' top={8 + rowheight * 4} />
            <AntdInputBox type='text' id='status' ref={ref => this.status = ref} label='完成情况' labelwidth='100' left='2' width='300' readOnly top={8 + rowheight * 5} />
