...
export default class Page1501 extends MyFormComponent {
  state = {
    myTable1: {
      data: [],  //某一页数据
      total: 0,  //行总数
  ...
  handleDeleteClick = async (e) => {
    ...
    this.myDeleteModal.setState({ visible: true, description: '是否确定删除【' + row[keyfield] + '】这个街道？' });
    return;
  }
  ...
  handleDeleteRow = async (row, rowindex) => {  //ddddddddddddddelete   
    ...
    xdata._reloadrow = 0;
    let p = {};
    p.sqlprocedure = 'demo1501b'; // "demo502c";
    p.data = [];
    p.data.push(xdata);
  ...
  handleSaveClick = async () => {  //sssssssssssave
    ...
    data._treeflag = 0;
    let p = {};
    p.sqlprocedure = 'demo1501b';  //"demo504a";
    p.data = [];
    p.data.push(data);
    if (addoredit != 'add') {
    ...
    if (rs.rows.length > 0 && (rs.rows[0]._error == undefined || rs.rows[0]._error == '')) { //数据验证
      ...
      this.setState({ addoredit: 'update', myWin1: false, myTable1: { ...table, total, rowindex, pageno } }, () => {
        setTimeout(() => {
          this.loadTableData();
          myNotice('街道记录已经保存，请刷新数据!', '', 200)
        })
      });
    } else {
      myNotice('街道编码重复，记录保存失败！', 'error');  //
    }
  }
  ...
  render() {
    ...
    const columns = [
      ...
      { dataIndex: 'streetno', title: '街道编号', width: '120px', align: 'center', fixed: 'left' },
      { dataIndex: 'streetname', title: '街道名称', width: '120px',align: 'center',fixed: 'left', ellipsis: true },
      { dataIndex: 'district', title: '所属县区', width: '100px', align: 'center', ellipsis: true },
      {
        ...
        render: (text, record, index) => <>
          ...
          <Popconfirm okText='确定' cancelText='取消' overlayStyle={{ width: 350 }} title='系统提示' description={`是否确定删除"${record.productname}"的这个街道？`}
    ...
    return (<>
      ...
      <Modal name='myWin1' title='街道详细信息' open={this.state.myWin1} width={480} forceRender centered maskClosable={false}
        cancelText='关闭' onCancel={() => { this.setState({ myWin1: false }) }}
        styles={{ position: 'relative', padding: 0, body: { overflowY: 'scroll', height: '150px', width: '450px', padding: 0, margin: 0 } }}
        ...
        <div style={{ position: 'relative' }}>  {/* 添加一个层，否则文本框需要大于30px才能输入 */}
          <Form name="myForm1" ref={ref => this.myForm1 = ref} autoComplete="off" onFinish={this.onFinish} initialValues={this.state.row}
            ...
            <AntdInputBox type='text' id='streetno' ref={ref => this.streetno = ref} label='街道编码' labelwidth='100' left='2' width='140' top={8 + rowheight * 0} />
            <AntdInputBox type='text' id='streetname' ref={ref => this.streetname = ref} label='街道名称' labelwidth='100' left='2' width='300' top={8 + rowheight * 1} />
            <AntdInputBox type='text' id='district' ref={ref => this.stationno = ref} label='所属区县' labelwidth='100' left='2' width='300' top={8 + rowheight * 2} />
