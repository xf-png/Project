...
export default class Page1503 extends Component {
  constructor(props) {
    super(props);
    //在这里初始化 state
    this.state = {      
  ...
  showOrders = async (item) => {  //必须加异步
    ...
    p.sqlprocedure='demo1503c';
    let rs = await reqdoSQL(p); //调用函数，执行存储过程。必须加await
    let rows=rs.rows.map((item, index) => {
      return(<li key={"item_"+index}>
        <span style={{display:'inline-block', width:70}}>{item.srecordno}</span>
        <span style={{display:'inline-block', width:100}}>{item.sweepdate}</span>
        <span style={{display:'inline-block', width:100}}>{item.sweeptime}</span>
        <span style={{display:'inline-block', width:70}}>{item.district}</span>

        <span style={{display:'inline-block', width:100}}>{item.streetname}</span>
        <span style={{display:'inline-block', width:70}}>{item.status}</span>
      </li>);
    })
    ...
    <div>
      <div style={{height:30, marginLeft:10, marginTop:10}}>查询员工工作记录<hr/></div>      
        ...
        <div style={{padding:10, marginTop:10, border:'1px solid #95B8E7', height:260, overflow:'auto'}}>
          <div><b>{this.state.selectedProduct.cleanername}</b>清扫信息（共***条）</div>
          <hr/>

          <span>序号</span>&nbsp;&nbsp;<span>记录编号</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span>清扫日期</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span>清扫时间</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span>区县所属</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span>街道名称</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span>完成情况</span>
