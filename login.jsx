...
export default class Login extends MyFormComponent {
        ...
        if (error == '') {
            ...
            if (rs.rows.length < 2) {
                ...
                this.setState({ isVisible: false, loginedFlag: true }, () => {
                    window.location.href = url;  //'index.js';  //重新定位到主页，即刷新主页，保证用户名称显示为新的值
                });
            }
        } else {
            notification.open({
    ...
    render() {
        ...
        return (
            <div>
                <Modal name='isVisible' title='用户登录' open={this.state.isVisible} height={245} width={367} forceRender centered maskClosable={false}
